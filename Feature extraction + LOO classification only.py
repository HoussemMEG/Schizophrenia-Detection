...
# absolute_sum_of_changes, approximate_entropy, \
#     autocorrelation, cid_ce, longest_strike_above_mean, longest_strike_below_mean, number_crossing_m, sample_entropy
...
class Classifier:
    def __init__(self,
        ...
        # Reading param
        self.path = kwargs.get('path', os.path.join(os.getcwd(), 'features'))
            ...
            for stim in self._stim:
                ...
                for subj, subj_data in data[stim].items():
                    if subj in self._remove_patient:
                        continue
                    subj_info = subj_data['subject_info']
                    ID.append(subj)
    ...
    def _feature_extraction(self, x: np.ndarray) -> np.ndarray:
        ...
        f = [
            ...
            # np.mean(x, axis=1),
            # np.max(x, axis=1),  # 69 % x0
            # np.min(x, axis=1),
            # np.max(x, axis=1) - np.min(x, axis=1),
            # np.argmin(x, axis=1),
            # np.argmax(x, axis=1),  # 74.1 x0
            # np.argmax(x, axis=1) - np.argmin(x, axis=1),  # 70.3 x0
            # np.argmax(x, axis=1) // 50 + np.argmin(x, axis=1) // 50,  # // 50,
            ...
            ## np.min(np.abs(x), axis=1),
            # np.argmax(np.abs(x), axis=1),
            ## np.argmin(np.abs(x), axis=1),
            # np.array([np.nonzero(row)[0][0] for row in x]),
            # np.array([np.nonzero(row)[0][-1] for row in x]),
            # np.array([row[np.nonzero(row)[0][0]] for row in x]),
            # np.array([row[np.nonzero(row)[0][-1]] for row in x]),
            # np.array([np.mean(row[np.where(row >= 0)]) for row in x]),
            ##### np.array([np.mean(row[np.where(row <= 0)]) for row in x]),
            # np.array([np.count_nonzero(row[np.where(row >= 0.05)]) for row in x]),
            # np.array([np.count_nonzero(row[np.where(row <= -0.05)]) for row in x]),
            # np.array([np.count_nonzero(row[np.where((row <= 0.5) & (row >= -0.5))]) for row in x]),
            # np.array([np.count_nonzero(row[np.where((row >= 0.05) | (row <= -0.05))]) for row in x]),
            # np.array([np.count_nonzero(row[np.where(row >= np.mean(np.abs(row)))]) for row in x]),
            # np.array([np.count_nonzero(row[np.where(row <= np.mean(np.abs(row)))]) for row in x]),  # 70%
            ## np.array([np.count_nonzero(row[np.where(row >= np.mean(row))]) for row in x]),
            ## np.array([np.count_nonzero(row[np.where(row <= np.mean(row))]) for row in x]),
            # np.array([np.count_nonzero(row[np.where((row <= np.std(row)) & (row >= -np.std(row)))]) for row in x]),
            ## np.array([np.count_nonzero(row[np.where((row >= np.std(row)) | (row <= -np.std(row)))]) for row in x]),
            # np.array([np.count_nonzero(row[np.where((row <= 3*np.std(row)) & (row >= -3*np.std(row)))]) for row in x]),
            ## np.array([np.count_nonzero(row[np.where((row >= 3*np.std(row)) | (row <= -3*np.std(row)))]) for row in x]),



            # utils.features(x, feat.absolute_sum_of_changes),
            # utils.features(x, feat.cid_ce, normalize=False),
            ## utils.features(x, feat.approximate_entropy, m=20, r=1),
            # utils.features(x, feat.autocorrelation, lag=50),
            # utils.features(x, feat.longest_strike_above_mean),
            # utils.features(x, feat.longest_strike_below_mean),
            # utils.features(x, feat.number_crossing_m, m=0),
            ## utils.features(x, feat.sample_entropy),

            np.argmin(x, axis=1) // self._n_freq * 2,                       # F_1
