...
class Reader:
    def __init__(self, path=None, info_path=None, subj_info_keys=None, use_eog=False):
        """
        ...
        self._data_path = os.path.join(path) if path is not None else None
        self._info_path = os.path.join(info_path) if info_path is not None else None
        self.files: Optional[List[str]] = os.listdir(self._data_path) if self._data_path is not None else None
    ...
    @staticmethod
    def data_to_mne(data, s_rate, channels, stim_types, subj, category):
        data = data * 1e-6
        # epochs but having only one epoch per stim
